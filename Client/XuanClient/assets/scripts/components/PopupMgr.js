...
cc.Class({
    ...
    onLoad: function () {
        ...
        this.zuduiSuccess= cc.find("Canvas/popups/zuduiSuccess");
       ...
       // this.addBtnHandler("dissolve_notice/btn_ok");
        ...
        this.node.on("dissolve_cancel", function (event) {
            var data = event.detail;
            var num = cc.vv.gameNetMgr.seats.length;
    ...
    closeAll: function () {
        ...
        this.zuduiSuccess.active = false;
    },
    ...
    showSettings: function () {
        ...
        if ((cc.vv.gameNetMgr.gamestate != "" && cc.vv.gameNetMgr.numOfGames == 0) || cc.vv.gameNetMgr.numOfGames >= 1 && cc.vv.gameNetMgr.conf.type != cc.vv.gameNetMgr.QUAN_MJ)
        {
            this.sqjsfj.active = true;
        }
        if (cc.vv.gameNetMgr.conf.type == cc.vv.gameNetMgr.QUAN_MJ)
        {
            this.sqjsfj.active = false;
            this.sqjsfj.parent.getChildByName("goldexit").active = true; 
        }
    },
    showUserInfo: function ()
    {
        if (this.userListImageLayout) {
            ...
            if (layout) {
                ...
                var userkuantprefab = cc.instantiate(cc.vv.mahjongmgr.UserKuantPrefabs);
    ...
    update: function (dt) {
        ...
        if (cc.vv.gameNetMgr.isOver && this._popuproot.active) {
            this.closeAll();
        }
        ...
        if (cc.vv.gameNetMgr.gamedissolvedata != null) {
                ...
                if (fromUserLabel) {
                    for (var k in seats) {
                        if (seats[k].userid == tongyiOrjujielist.fromUser) {
                            ...
                            fromUserLabel.string = "玩家" + namestr + "申请解散房间，全部玩家同意或三分钟后房间将解散";
                        }
                    }
                }
            }
            ...
            //同意
            var resindex = 0;
            if(cc.vv.gameNetMgr.userIcons == null){
                cc.vv.gameNetMgr.userIcons = cc.vv.userIcons;
            }
            for (var i = 0; i < tongyiOrjujielist.tongyiUsers.length; i++) {
                ...
                if (this._ResUserinfo != null) {
                    ...
                    cc.vv.mahjongmgr.getAllUserIcon();
                    if (cc.vv.gameNetMgr.userIcons != null && cc.vv.gameNetMgr.userIcons[this._ResUserinfo[resindex].userid]!=null && cc.vv.gameNetMgr.userIcons[this._ResUserinfo[resindex].userid].node != null) {
