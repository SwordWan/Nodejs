cc.Class({
    ...
    onLoad: function () {
        ...
        this.node.on('push_notice', function (data) {
            return;
            var gametype = cc.vv.gameNetMgr.conf.type;

            if (gametype == cc.vv.gameNetMgr.XZDD_GAME || gametype == cc.vv.gameNetMgr.WANGBA_GAME || cc.vv.gameNetMgr.conf.type == cc.vv.gameNetMgr.QUAN_MJ)
            {
                    if (this._guohu)
                        this._guohu.active = false;
                    return;
