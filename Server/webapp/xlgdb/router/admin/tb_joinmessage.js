...
const db = require('../../dal/tb_joinmessage.js');
...
router.get('/view/list', async function (req, res) {
    ...
    options.url_page_p1 = '/admin/tb_joinmessage/view/list?p=1';
    options.url_page_p2 = '/admin/tb_joinmessage/view/list?p=';
    options.url_page_p3 = '/admin/tb_joinmessage/view/list?p=';
    options.url_page_p4 = '/admin/tb_joinmessage/view/list?p=';
    options.cur = "tb_joinmessage";
    // options.menu = menu.get(req.session.admin.is_admin);
    options.menu = menu.get(0);
    options.title = "需要自己修改的";
    options.layout = 'admin/layout';
    res.render('admin/tb_joinmessage/list', options);
});
...
router.get('/view/create', async function (req, res) {
    ...
    options.title = "需要自己修改的";
    options.layout = 'admin/layout';
    options.posturl = '/admin/tb_joinmessage/ajax/create';
    res.render('admin/tb_joinmessage/view', options);
});
...
router.get('/view/update', async function (req, res) {
    ...
    try {
        do {
            ...
            let params = {};
            let rows = await db.tb_joinmessage_select(conn, params);
            if (rows.length == 0) {
                options.errcode = 1;
    ...
    options.title = "需要自己修改的";
    options.layout = 'admin/layout';
    options.posturl = '/admin/tb_joinmessage/ajax/update';
    res.render('admin/tb_joinmessage/view', options);
});
...
router.post('/ajax/create', async function (req, res) {
    ...
    try {
        do {
            ...
            let row = {};
            let res = db.tb_joinmessage_add(conn, row);
...
router.post('/ajax/update', async function (req, res) {
    ...
    try {
        do {
            ...
            let row = {};
            let res = db.tb_joinmessage_update(conn, row);
            if (res.affectedRows == 0) {
                result.errcode = -1;
                result.errmsg = '更新失败';
            }
...
router.get('/ajax/delete', async function (req, res) {
    ...
    try {
        do {
            ...
            let row = {};
            let res = db.tb_joinmessage_delete(conn, row);
            if (res.affectedRows == 0) {
                result.errcode = -1;
                result.errmsg = '删除失败';
            }
...
async function getCount(conn, req) {
    var row = {};
    return await db.tb_joinmessage_count(conn, row);
}

async function getList(conn, req, page, size) {
    var row = { page: page, size: size };
    return await db.tb_joinmessage_list(conn, row);
}
